...
var convolutional_depthwise_1 = require("./convolutional_depthwise");
test_utils_1.describeMathCPUAndGPU('depthwiseConv2d', function () {
    var x4by4Data = [[[
                ...
                var _loop_3 = function (depthMultiplier) {
                    ...
                    it(testTitle, function () {
                        ...
                        if (depthMultiplier === 1) {
                            kernel = tfjs_core_1.tensor4d([1, 0, 0, -1], [2, 2, 1, 1]);
                        }
                        else if (depthMultiplier === 2) {
                            kernel = tfjs_core_1.tensor4d([1, -1, 0, 0, 0, 0, -1, 1], [2, 2, 1, 2]);
                        }
                        var y = convolutional_depthwise_1.depthwiseConv2d(x, kernel, [stride, stride], 'valid', dataFormat);
                        var yExpected;
                        if (stride === 1) {
                            if (depthMultiplier === 1) {
                            ...
                            if (depthMultiplier === 1) {
                                ...
                                yExpected = tfjs_core_1.tensor4d([[[[-30, -30], [30, 30]], [[30, 30], [-30, -30]]]], [1, 2, 2, 2]);
                            }
                        }
                        if (dataFormat !== 'channelsFirst') {
    ...
    it('Non-4D kernel leads to exception', function () {
        ...
        expect(function () { return convolutional_depthwise_1.depthwiseConv2d(x, tfc.zeros([1, 2, 2]), [1, 1]); })
            .toThrowError(/.* is required to be 4-D, but is instead 3-D/);
    });
});
test_utils_1.describeMathCPU('DepthwiseConv2D-Symbolic', function () {
    ...
    it('Serialization round trip', function () {
        var layer = tfl.layers.depthwiseConv2d({ kernelSize: 3, depthMultiplier: 4, activation: 'relu' });
        ...
        var layerPrime = tfl.layers.depthwiseConv2d(tsConfig);
        var configPrime = layerPrime.getConfig();
...
test_utils_1.describeMathCPUAndGPU('DepthwiseConv2D-Tensor:', function () {
    var x4by4Data = [[[
                [10, 30, 50, 70], [20, 40, 60, 80], [-10, -30, -50, -70],
                [-20, -40, -60, -80]
            ]]];
